...
當所有人履歷上都寫「會 React、會 Node」，**你多寫一個「做過 RAG 文件問答系統，部署在 Vercel」會直接在一堆履歷中跳出來**。

### 4. 進可攻退可守

學了 LLM API + RAG，既能往「AI 工程師」深耕（接 fine-tune、agent、向量 DB 設計），也能回到普通後端 / 前端工作把 AI feature 加進產品。**對實習生這是最高 ROI 的方向之一**。

## 二、主流 LLM API：三大供應商總覽

### 1. Anthropic Claude

由 Anthropic 提供，主要模型家族包括 Sonnet、Opus、Haiku 系列。**特色**：長 context、推理品質強、安全性著稱。SDK 包括 `@anthropic-ai/sdk`（Node）與 `anthropic`（Python）。具體模型版本名稱與費率，**請查 Anthropic 官方最新資訊**，因為命名與版本變動快。

### 2. OpenAI（ChatGPT 背後）

GPT-4o、GPT-5、o1、o3 系列。**特色**：生態最大、社群教學最多、跨領域能力強、語音與圖像整合廣。SDK 包括 `openai`（Python 與 Node）。模型版本變動快，本文僅以「最新模型」「進階版」「輕量版」帶過，實際版本名稱請查官方。

### 3. Google Gemini

Gemini Pro、Gemini Flash 系列。**特色**：與 Google 生態整合好、長 context、多模態能力強。在台灣許多公司因 Google Cloud 折扣成為主力。
...
|------|-----------------|--------|---------------|-----------|
| 強項 | 長 context、推理 | 生態大、跨領域 | Google 整合、多模態 | 開源、可自託管 |
| Context window | 長（百萬級規模） | 長 | 長 | 視部署而定 |
| 多模態 | 圖、文 | 圖、文、語音 | 圖、文、影片 | 視版本 |
| SDK | Node、Python | Node、Python、其他 | Node、Python | 多種 |
| 價格 | 請查官方最新 | 請查官方最新 | 請查官方最新 | 自託管成本 |
| 適合用途 | 文件分析、agent、長對話 | 通用、工具呼叫 | Google 生態整合 | 私有部署、研究 |
...
1. 到該供應商官網（Anthropic、OpenAI、Google AI Studio）註冊帳號。
...
並在 `.gitignore` 加上 `.env`。實習生最常見的 GitHub 慘案是把 key 推上去，幾分鐘內就被機器人爬走、產生大筆費用。**Anthropic、OpenAI、GitHub 都有自動偵測機制，發現後會通知你 revoke**，但你要先做好基本防護。
...
message = client.messages.create(
    model="claude-latest-model",  # 實際模型名稱請查官方
    max_tokens=1024,
    messages=[
...
const completion = await client.chat.completions.create({
  model: "gpt-latest-model", // 實際模型名稱請查官方
  messages: [
...
**入門最熱門的 LLM 框架**，提供 Chain、Agent、Memory、Tool 抽象。優點是文件多、社群大；缺點是 2024 起被部分工程師批評「過度抽象」，簡單任務用直接 SDK 反而清爽。

**適合**：學習階段、需要快速串多個工具的 agent。
...
**前端與 Next.js 整合最佳**，提供 `useChat`、`useCompletion` 等 React hooks，串流 UI 寫法極簡。

**適合**：用 Next.js 做 AI chat UI、消費型 AI 產品的前端串接。

### 4. Pydantic AI

**Python 圈 2024 後的新興工具**，主打 type-safe agent 寫法、與 Pydantic 整合佳。

**適合**：希望 agent 行為被型別約束的 Python 專案。
...
- **過長 context**：把 50 個 chunk 全塞進 prompt，模型反而 confuse。
- **embedding 模型不一致**：建索引用 A 模型、查詢用 B 模型，相似度全錯。

### 3. embedding model 選擇

- **OpenAI text-embedding-3**：通用、效果穩定。
- **Voyage**：在 RAG benchmark 上表現好，Anthropic 推薦。
- **Cohere**：多語言不錯。
- **開源 sentence-transformers**：免費、可自託管。

## 七、向量資料庫：選擇邏輯

### 1. pgvector（PostgreSQL 擴充，最簡單入門）

如果你已經在用 PostgreSQL，**直接裝 pgvector 擴充就有向量功能**，不用學新 DB、不用多開一個服務。Supabase、Neon、Zeabur 的 PostgreSQL 都能直接啟用。
...
- `embedding <=> %s::vector` 是 pgvector 的 cosine distance 運算子。
- 真實專案會把 chunk 切割、metadata、檔案來源、版本一起存。
- 真實專案應該批次 embed（避免逐筆 API 呼叫）、加 cache、加錯誤處理。
- 模型名稱寫實際版本要查官方，本範例用代稱。
...
便宜、輕量模型（如各家的 Haiku / Flash / mini 系列）對「分類、抽取、簡單問答」就夠。**不要全部任務都用最大模型**，成本會炸。
...
非即時任務（報表、批次資料抽取、夜間摘要）可用 batch API，半價或更便宜，延遲幾小時不影響。
...
VS Code 與 JetBrains 的 AI 補全外掛。Copilot Chat 還支援對話。學生有教育版優惠（請查 GitHub 官方最新方案）。

### 3. Claude Code

Anthropic 出的 CLI agent，能跑指令、改檔案、處理長任務。終端機派工程師的選擇。

### 4. Zed

新一代高效能編輯器，內建 AI 整合，速度極快，是 VS Code 的競爭者。
...
> **薪資提醒**：AI 相關實習在台灣 2026 年薪水普遍偏高，台北月薪常見從 NT$30,000 起跳，外商或大型 AI 新創甚至 NT$50,000 以上。但**實際數字以公司公告為準**，這提供的是判斷「值得投入多少時間」的參考。
...
- embedding 是「把語意變成向量」，相似的東西向量距離近。
...
讓 LLM 自己決定要呼叫哪些工具（搜尋、計算、SQL）。Anthropic 的 tool use、OpenAI 的 function calling 都是入口。

### 2. Fine-tuning

當 prompt 工程已到極限、且你有大量領域資料，可考慮 fine-tune。LoRA、QLoRA 是常見技巧。

### 3. 評估（Evaluation）

LLM 應用最難的是「怎麼知道我改的版本變好還是變差」。學 LLM-as-judge、benchmark 設計、A/B 測試。
...
2. **成本與效能要有概念**：實習公司不怕你用 LLM，怕你不會控制 token 與成本。會選對模型、會用 cache、會設 max_tokens 的實習生最被信任。
