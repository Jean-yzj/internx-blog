...
### 1. Notion 是「結構化筆記」
Google Docs 是線性文件，找一個三個月前的會議紀錄要爬 30 個檔案。Notion 用 Database 把所有筆記變成「可篩選、可排序、可標籤」的結構化資料，**找東西的速度差 10 倍**。
...
這個動作會讓你在主管心中的形象從「實習生」升級成「能獨立 own 進度的人」。**會議效率提升 50%，1on1 從 30 分鐘變 15 分鐘**，主管會更喜歡你。

## 八、實習結束：把筆記變成 Portfolio

實習最後一週，做這件事比寫離職信重要 10 倍。
...
**三個關鍵原則**：
1. **每天 10 分鐘 > 每週 1 小時**：頻率比深度重要。一個有缺漏但每天寫的 Daily Log，比完美但只寫三次的好十倍。
